...
function parseInstruction(instruction, transforms) {
  ...
  let bottomLeftCoordinates = coordinates[1].split(',')
  ...
  let bottomRight = {
    x: Number.parseInt(bottomLeftCoordinates[0].trim()),
    y: Number.parseInt(bottomLeftCoordinates[1].trim())
  }

  return (grid) => {
    operateOnGrid(grid, topLeft, bottomRight, fun)
  }
}
...
function getFinalLightGrid(instructions, transforms) {
  let lightsGrid = createGrid(1000)
